...
impl SymbolTable {
    ...
    fn define_builtin_types(&mut self) {
        ...
        for type_name in &["i8", "i16", "i32", "i64", "u8", "u16", "u32", "u64", "f32", "f64", "bool", "char", "str"] {
...
impl TypeChecker {
                ...
                if let Some(annotated_ty) = ty {
                    ...
                    if !self.types_compatible(&value_type, &expected_ty) {
                        return Err(Error::new(
                            ...
                            name.span,
                            format!("Type mismatch: expected {:?}, found {:?}", expected_ty, value_type),
                        ));
                    }
                }
                ...
                if let Some(expected) = &self.current_function_return {
                    if !self.types_compatible(&return_type, expected) {
                        return Err(Error::new(
                            ...
                            format!("Return type mismatch: expected {:?}, found {:?}", expected, return_type),
                        ));
                    }
                }
            }
    ...
    fn check_expr(&mut self, expr: &Expr) -> Result<ResolvedType> {
        match &expr.kind {
            ...
            ExprKind::Call { func, args: _ } => {
                ...
                match func_ty {
                    ResolvedType::Function { return_type, .. } => {
                        Ok(*return_type)
                    }
                    _ => {
    ...
    fn check_binary_op(&self, op: BinOp, left: &ResolvedType, right: &ResolvedType, span: Span) -> Result<ResolvedType> {
        use BinOp::*;
        use PrimitiveType::*;
            ...
            Add | Sub | Mul | Div | Rem => {
                ...
                if self.is_numeric(left) && self.types_compatible(left, right) {
                    ...
                    Err(Error::new(
                        ErrorKind::TypeMismatch,
                        span,
                        format!("Arithmetic operation requires numeric types, found {:?} and {:?}", left, right),
                    ))
                }
            }
            Eq | Ne | Lt | Le | Gt | Ge => {
                ...
                if self.types_compatible(left, right) {
                    ...
                    Err(Error::new(
                        ErrorKind::TypeMismatch,
                        span,
                        format!("Comparison requires compatible types, found {:?} and {:?}", left, right),
                    ))
                }
            }
            And | Or => {
                ...
                if *left == ResolvedType::Primitive(Bool) && *right == ResolvedType::Primitive(Bool) {
                    ...
                    Err(Error::new(
                        ErrorKind::TypeMismatch,
                        span,
                        format!("Logical operation requires bool, found {:?} and {:?}", left, right),
                    ))
                }
            }
    ...
    fn resolve_type(&self, ty: &Type) -> Result<ResolvedType> {
        match ty {
            ...
            Type::Tuple(types) => {
                let resolved: Result<Vec<_>> = types.iter().map(|t| self.resolve_type(t)).collect();
    ...
    fn is_numeric(&self, ty: &ResolvedType) -> bool {
        matches!(
            ty,
            ResolvedType::Primitive(PrimitiveType::I8 | PrimitiveType::I16 | PrimitiveType::I32 | PrimitiveType::I64 |
